...
public:
    ...
    int64 GetBlockMoneyDestroy() const
    {
        int64 nTotalDestroy = 0;
        for (const CTransaction& tx : vtx)
        {
            if (tx.sendTo.IsTemplate() && tx.sendTo.GetTemplateId().GetType() == TEMPLATE_ACTIVATE)
            {
                nTotalDestroy += tx.nAmount;
            }
        }
        return nTotalDestroy;
    }
    uint256 BuildMerkleTree(std::vector<uint256>& vMerkleTree) const
    {
        ...
        for (size_t nSize = vtx.size(); nSize > 1; nSize = (nSize + 1) / 2)
        {
            for (size_t i = 0; i < nSize; i += 2)
            {
                ...
                vMerkleTree.push_back(ibrio::crypto::CryptoHash(vMerkleTree[j + i], vMerkleTree[j + i2]));
            }
            j += nSize;
        }
...
public:
    CBlockEx() {}
    CBlockEx(const CBlock& block, const std::vector<CTxContxt>& vTxContxtIn = std::vector<CTxContxt>(), const uint256& nTrustIn = uint256())
      : CBlock(block), vTxContxt(vTxContxtIn), nBlockTrust(nTrustIn)
    {
