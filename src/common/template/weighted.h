...
{
public:
    CTemplateWeighted(const uint8 nRequiredIn = 0, const std::map<ibrio::crypto::CPubKey, uint8>& mapPubKeyWeightIn = std::map<ibrio::crypto::CPubKey, uint8>());
...
protected:
    uint8 nRequired;
    std::map<ibrio::crypto::CPubKey, uint8> mapPubKeyWeight;
};
