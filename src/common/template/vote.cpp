...
CTemplateVote::CTemplateVote(const CDestination& destDelegateIn, const CDestination& destOwnerIn, const uint32 nCycleIn, const uint64 nNonceIn)
  : CTemplate(TEMPLATE_VOTE), destDelegate(destDelegateIn), destOwner(destOwnerIn), nCycle(nCycleIn), nNonce(nNonceIn)
{
}
...
void CTemplateVote::GetTemplateData(ibrio::rpc::CTemplateResponse& obj, CDestination&& destInstance) const
{
    obj.vote.strDelegate = (destInstance = destDelegate).ToString();
    obj.vote.strOwner = (destInstance = destOwner).ToString();
    obj.vote.nCycle = nCycle;
    obj.vote.nNonce = nNonce;
}
...
bool CTemplateVote::ValidateParam() const
{
    ...
    if (mapInterestRule.count(nCycle) == 0)
    {
        return false;
    }
    return true;
...
bool CTemplateVote::SetTemplateData(const std::vector<uint8>& vchDataIn)
{
    ...
    try
    {
        is >> destDelegate >> destOwner >> nCycle >> nNonce;
    }
...
bool CTemplateVote::SetTemplateData(const ibrio::rpc::CTemplateRequest& obj, CDestination&& destInstance)
{
    ...
    nCycle = obj.vote.nCycle;
    nNonce = obj.vote.nNonce;
    return true;
}
...
void CTemplateVote::BuildTemplateData()
{
    ...
    os << destDelegate << destOwner << nCycle << nNonce;
}
