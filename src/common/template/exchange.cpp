    ...
    fork_s(fork_s_)
{
    ...
    nId = CTemplateId(nType, ibrio::crypto::CryptoHash(vchData.data(), vchData.size()));
}
...
void CTemplateExchange::GetTemplateData(ibrio::rpc::CTemplateResponse& obj, CDestination&& destInstance) const
{
    ...
    obj.exchange.strSpend_M = ibrio::CAddress(destSpend_m).ToString();
    obj.exchange.strSpend_S = ibrio::CAddress(destSpend_s).ToString();
}
...
bool CTemplateExchange::SetTemplateData(const ibrio::rpc::CTemplateRequest& obj, CDestination&& destInstance)
{
    ...
    ibrio::CAddress address_m(addr_m);
    ibrio::CAddress address_s(addr_s);
