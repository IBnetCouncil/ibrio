...
CTemplateWeighted::CTemplateWeighted(const uint8 nRequiredIn, const std::map<ibrio::crypto::CPubKey, uint8>& mapPubKeyWeightIn)
...
void CTemplateWeighted::GetTemplateData(ibrio::rpc::CTemplateResponse& obj, CDestination&& destInstance) const
{
    obj.weighted.nRequired = nRequired;
    for (const auto& keyweight : mapPubKeyWeight)
    {
        ibrio::rpc::CTemplatePubKeyWeight pubkey;
...
bool CTemplateWeighted::SetTemplateData(const ibrio::rpc::CTemplateRequest& obj, CDestination&& destInstance)
{
