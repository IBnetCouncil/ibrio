...
{
    if (!config.Load(argc, argv, GetDefaultDataDir(), "ibrio.conf") || !config.PostLoad())
    {
    ...
    {
        cout << "Ibrio version is v" << VERSION_STR << ", git commit id is " << GetGitVersion() << endl;
        return false;
    }
    ...
    if ((config.GetModeType() == EModeType::MODE_SERVER || config.GetModeType() == EModeType::MODE_MINER)
        && log.SetLogFilePath((pathData / "ibrio.log").string())
    ...
    {
        cerr << "Failed to open log file : " << (pathData / "ibrio.log") << "\n";
        return false;
    }
    ...
    {
        int64 nMaxBlockRewardTxCount = CBlockChain::GetBlockInvestRewardTxMaxCount();
        CCheckRepairData check(pathData.string(), config.GetConfig()->fTestNet, config.GetConfig()->fOnlyCheck, config.GetConfig()->fAddrTxIndex, nMaxBlockRewardTxCount);
        if (!check.CheckRepairData())
        {
            if (config.GetConfig()->fOnlyCheck)
            {
                StdError("Ibrio", "Check data fail.");
            }
            else
            {
                StdError("Ibrio", "Check and repair data fail.");
            }
            return false;
        }
        if (config.GetConfig()->fOnlyCheck)
        {
            StdLog("Ibrio", "Check data complete.");
            return false;
        }
        StdLog("Ibrio", "Check and repair data complete.");
    }

#if !defined(WIN32) && !defined(__APPLE__)
    StdLog("Ibrio", "malloc_trim: %d.", malloc_trim(0));
#endif
    ...
    StdLog("IbrioStartup", "Initialize: ibrio version is v%s, git commit id: %s", VERSION_STR.c_str(), GetGitVersion());
        ...
        case EModuleType::LOCK:
        {
            if (!TryLockFile((config.GetConfig()->pathData / ".lock").string()))
            {
                cerr << "Cannot obtain a lock on data directory " << config.GetConfig()->pathData << "\n"
                     << "Ibrio is probably already running.\n";
                return false;
            }
            break;
        }
...
{
    // Windows: ./.ibrio or C:\.ibrio
    // Mac: ~/Library/Application Support/Ibrio
    // Unix: ~/.ibrio
...
bool CBbEntry::RunInBackground(const path& pathData)
{
    ...
    if (pid > 0)
    {
        FILE* file = fopen((pathData / "ibrio.pid").string().c_str(), "w");
        if (file)
        {
            fprintf(file, "%d\n", pid);
            fclose(file);
...
{
#ifndef _WIN32
    boost::filesystem::remove(pathData / "ibrio.pid");
#endif
}
